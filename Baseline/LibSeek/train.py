...
def train(epochs) -> None:
    ...
    position = np.zeros(shape=(size_app, 20), dtype=np.uint16)
    ...
    for u in range(size_app):
        ...
        new_xiabiao = []
        # 推荐前20个TPL
        for lib_order_id in xiabiao[:20]:
            new_xiabiao.append(lib_order_id)
        position[u, :20] = np.array(new_xiabiao)

    del xiabiao, pre_u, new_xiabiao

    # 保存预测结果
    utility.utils.ensure_dir(args.rec_output + 'fold%s_rmv%s/' % (fold_rmv[0], fold_rmv[1]))        # 这里保存的是tpl_list不为0的预测结果
